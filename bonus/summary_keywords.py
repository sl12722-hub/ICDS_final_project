...
def _normalize_messages(messages: list[dict[str, object]], limit: int = DEFAULT_HISTORY_LIMIT) -> list[dict[str, str]]:
...
def generate_summary(messages: list[dict[str, object]], max_items: int = 2) -> str:
    ...
    ordered_contents = [content for _score, _index, content in sorted(top_messages, key=lambda item: item[1])]
